...
# Remote Multiprocess socket handler
...
try:
    ...
    if (len(cut_out_masks) < 1):
        print ('No cut out masks available.')
        use_cut_outs = False    
    else:
        print ('(x,y,width,height) of each mask:')
        print (cut_out_masks)
except Exception as e:
    print(e)
    print('Could not read cut out masks file.')
    use_cut_outs = False
    ...
    try:
        ...
        if (np.isnan(min_temp) or np.isnan(max_temp)):
            ...
            if (type3_error_count > MAX_ERRORS):
                ...
                sys.exit(3)    # TODO: revisit this
        type3_error_count = 0
        
        print ("min : {} , max : {}".format (min_temp, max_temp))
        data_ptr = data_ptr.reshape(ROWS, COLS)
        #spurious_mask = [4,10,12,18]
        #data_ptr[14:20, 2:28] = 40.0  # 0.0
        ...
        #--- mask out spurious heat sources ----
        # first row range, then column range
        #frame [100:200, 5:300] = 255  # use 0 for testing
        if (use_cut_outs):
            for m in cut_out_masks:
                frame[m[1]:(m[1]+m[3]), m[0]:(m[0]+m[2])] = 255  # 0 for testing  
